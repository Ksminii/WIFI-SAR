...
class SimulationVisualizer:
    ...
    def update(self, env: SimulationEnvironment, true_pos: np.ndarray, reported_pos: np.ndarray, simulation_time: float, current_rssi: float, status: str = "Moving", next_waypoint: np.ndarray = None):
        # 첫 업데이트일 때 시작 시간 초기화
        ...
        if next_waypoint is not None:
            self.ax.plot(next_waypoint[0], next_waypoint[1], 'go', markersize=10, mfc='none', mew=2, label='Next Waypoint', zorder=3)
            self.ax.plot([true_pos[0], next_waypoint[0]], [true_pos[1], next_waypoint[1]], 'y--', alpha=0.7, linewidth=1.5)
        
        # 목표 (hotspot)
        self.ax.plot(env.hotspot_pos[0], env.hotspot_pos[1], 'r*', markersize=20, label='Hotspot (Goal)', zorder=4)
        
        # 드론에서 목표까지 선 그리기
        self.ax.plot([true_pos[0], env.hotspot_pos[0]], [true_pos[1], env.hotspot_pos[1]], 'g--', alpha=0.3, linewidth=1)
        ...
        self.ax.set_title(f"[{status}] Time: {simulation_time:.1f}s | Distance: {distance_to_goal:.1f}m | RSSI: {current_rssi:.1f} dBm | Speed: {speed_display}")
...
class SimulationRunner:
    ...
    def run_single(self, visualizer: SimulationVisualizer = None) -> SimResult:
        ...
        while simulation_time < self.params.TIME_LIMIT:
            ...
            # 🔍 알고리즘 상태 디버그 정보
            if loop_count <= 10:
                print(f"[Loop {loop_count}] State: {algo.state} | RSSI: {current_rssi:.2f} dBm | Waypoint: ({algo.waypoint[0]:.1f}, {algo.waypoint[1]:.1f})")
            
            # waypoint 계산 중 상태 표시
            if visualizer:
                visualizer.update(env, true_pos, None, simulation_time, current_rssi, status="Computing Waypoint", next_waypoint=algo.waypoint)
           ...
           # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
            ...
            # 🔍 첫 10개 루프 - 이동 벡터 상세 정보
            if loop_count <= 10:
                print(f"[Loop {loop_count}] reported_pos: ({reported_pos[0]:.1f}, {reported_pos[1]:.1f}) | waypoint: ({algo.waypoint[0]:.1f}, {algo.waypoint[1]:.1f}) | move_vector: ({move_vector[0]:.1f}, {move_vector[1]:.1f})")
            ...
            for step in range(num_steps):
                ...
                # ✅ 중요: algo.pos 업데이트하여 waypoint 계산 기준 최신 유지
                algo.update_position(reported_pos)
                    
                simulation_time += time_to_travel / num_steps
                
                # 시각화 업데이트 (매 프레임마다 부드럽게)
                if visualizer:
                    visualizer.update(env, true_pos, None, simulation_time, current_rssi, status="Moving", next_waypoint=algo.waypoint)
            ...
            if pause_time > 0:
                ...
                for _ in range(num_pause_frames):
                    ...
                    if visualizer:
                        visualizer.update(env, true_pos, None, simulation_time, current_rssi, status="Scanning RSSI", next_waypoint=algo.waypoint)
        ...
        if visualizer and algo.is_finished:
            visualizer.update(env, true_pos, None, simulation_time, current_rssi, status="SEARCH Finish", next_waypoint=algo.waypoint)
...
def main():
    ...
    if mode == '1':
        ...
        visualizer = SimulationVisualizer(time_scale=time_scale)
