...
class HomingAlgorithm:
    def __init__(self, start_pos: np.ndarray, params: SimParams):
        ...
        self.waypoint, self.path = self.pos.copy(), [start_pos.copy()]
        self.params, self.is_finished = params, False
        self.state = "SPIRAL"
    ...
    def decide_action(self, rssi: float):
        ...
        if rssi > self.best_known_signal: self.best_known_signal, self.best_known_pos = rssi, self.pos.copy()

        if self.state == "SPIRAL": self._execute_spiral(rssi)
        elif self.state == "ADAPTIVE_ASCENT": self._execute_adaptive_ascent(rssi)
        elif self.state == "ESCAPING": self._execute_escaping(rssi)
    ...
    def _execute_adaptive_ascent(self, rssi: float):
        if rssi > self.last_signal: self.stuck_counter = 0
        else:
    ...
    def get_total_distance(self) -> float:
        return np.sum(np.linalg.norm(np.diff(np.array(self.path), axis=0), axis=1))
...
class SimulationRunner:
    ...
    def run_single(self, visualizer: SimulationVisualizer = None) -> SimResult:
        ...
        # 4. 성민 >> rayleigh 페이딩 추가
        
        # 5. 성민 >> rician fading 추가
        
        # 6. 성민 >> 이 두 페이딩 모델을 각각 적용할 수 있도록 코드 수정해야 함 
