 ...
 *
 * [사용법]
 *   ./simple serial:///dev/ttyAMA0:57600  (실제 드론)
 *   ./simple udp://:14540                 (SITL 테스트)
 *
 * [설정 변수]
 ...
 * [수정 P1] target_ssid를 필수로 전달받아 정확한 SSID 스캔
 */
float get_signal_strength(const std::string& target_ssid,
    ...
    // 평균 계산 (outlier 제거)
    if (samples.empty()) {
        std::cout << "[신호 측정] SSID '" << target_ssid << "'를 찾았으나 샘플이 없습니다." << std::endl;
        return -999.0f;
    }
...
/**
 * NED 로컬 좌표로 이동
 * [수정 P2] MAVSDK Offboard는 자동으로 위치 명령을 지속 전송 (2Hz 이상)
 */
bool goto_position_ned(std::shared_ptr<Offboard> offboard,
    ...
    // [P2] Offboard 모드로 위치 명령 전송 (MAVSDK가 자동으로 지속 전송)
...
/**
 * 중앙/북/남/동/서 5방향 탐색 (효율적 로직)
 * [수정 P3] 비효율적인 8회 이동 로직을 6회 이동 로직으로 변경
 */
SearchResult search_five_directions(std::shared_ptr<Offboard> offboard,
                                    std::shared_ptr<Telemetry> telemetry,
                                    float search_distance = 10.0f,
                                    const std::string& target_ssid = "Victim_Phone_WiFi") {

    std::cout << "\n============================================================" << std::endl;
    std::cout << "5방향 신호 탐색 시작 (탐색 거리: " << search_distance << "m)" << std::endl;
    std::cout << "============================================================\n" << std::endl;
    ...
    // 0. 중앙(원점) 복귀 및 측정
    std::cout << "원점(중앙)으로 이동" << std::endl;
    goto_position_ned(offboard, telemetry, home_north, home_east, home_down, 60);
    measure_signal("중앙");

    // 1. 북쪽 이동 및 측정
    std::cout << "북쪽으로 이동" << std::endl;
    goto_position_ned(offboard, telemetry, home_north + search_distance, home_east, home_down, 60);
    measure_signal("북쪽");

    // 2. 남쪽 이동 및 측정
    std::cout << "남쪽으로 이동" << std::endl;
    goto_position_ned(offboard, telemetry, home_north - search_distance, home_east, home_down, 60);
    measure_signal("남쪽");

    // 3. 중앙 복귀
    std::cout << "원점(중앙) 복귀" << std::endl;
    goto_position_ned(offboard, telemetry, home_north, home_east, home_down, 60);

    // 4. 동쪽 이동 및 측정
    std::cout << "동쪽으로 이동" << std::endl;
    goto_position_ned(offboard, telemetry, home_north, home_east + search_distance, home_down, 60);
    measure_signal("동쪽");

    // 5. 서쪽 이동 및 측정
    std::cout << "서쪽으로 이동" << std::endl;
    goto_position_ned(offboard, telemetry, home_north, home_east - search_distance, home_down, 60);
    measure_signal("서쪽");
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "수동으로 이륙 후 'GUIDE' 모드로 변경하고" << std::endl;
    ...
    for (int round_num = 1; round_num <= MAX_ROUNDS; round_num++) {
        ...
        // 5방향 탐색
        auto best_result = search_five_directions(offboard, telemetry, SEARCH_DISTANCE, TARGET_SSID);
        ...
        if (best_result.direction == "중앙") {
            std::cout << "\n'중앙' 지점 신호(" << best_result.signal << " dBm)가 가장 강합니다." << std::endl;
            std::cout << "신호 정점에 도달한 것으로 판단, 탐색을 종료합니다." << std::endl;
            break;
        }
